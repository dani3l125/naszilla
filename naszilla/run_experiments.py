...
def run_experiments(args, save_dir):
    ...
    for compression_method in ['k_means_coreset_orig_dist', 'k_means_coreset', 'uniform', 'k_medoids']:
        ...
        for algo_name in algorithm_results.keys():
            ...
            ax1.errorbar(x=np.arange(1, 301, 1)[9:], y=result[9:], yerr=algorithm_results[algo_name][1][9:],
                         fmt='-', errorevery=10)
            ax2.plot(np.arange(10, 301, 10), sota_val_result[9:], 'o-', label=label_mapping[algo_name]+', SOTA')
            ax2.plot(np.arange(10, 301, 10), val_result[9::10], '^-', label=label_mapping[algo_name]+', ours')
            ax2.errorbar(x=np.arange(1, 301, 1)[9:], y=val_result[9:], yerr=algorithm_val_results[algo_name][1][9:],
                         fmt='-', errorevery=10)
            np.save(
