...
def run_experiments(args, save_dir):
    ...
    for i in range(trials):
        ...
        result_std = np.zeros_like(results[0].T[0])
        result_mean_val = np.zeros_like(val_results[0].T[0])
        result_std_val = np.zeros_like(val_results[0].T[0])
        for result in results:
            result_mean = np.add(result_mean, result.T[1])
        result_mean /= len(results)
        for result in results:
            result_std = np.add(result_std, np.abs(result.T[1] - result_mean))
        result_std /= len(results)
        for result in val_results:
            result_mean_val = np.add(result_mean_val, result.T[1])
        result_mean_val /= len(results)
        for result in val_results:
            result_std_val = np.add(result_std_val, np.abs(result.T[1] - result_mean_val))
        result_std_val /= len(results)
        if args.save_sota:
            np.save(f'sota_results/{args.algo_params}_{args.dataset}_mean.npy_val', result_mean)
            np.save(f'sota_results/{args.algo_params}_{args.dataset}_std.npy_val', result_std)
            np.save(f'sota_results/{args.algo_params}_{args.dataset}_mean.npy_val', result_mean_val)
            np.save(f'sota_results/{args.algo_params}_{args.dataset}_std.npy_val', result_std_val)
